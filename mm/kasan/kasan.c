...
void *__memmove(void *dst, const void *src, size_t len)
{
	...
	return ____memmove(dst, src, len);
}
...
#undef __memset
void *__memset(void *p, int c, size_t len)
{
	__asan_storeN((unsigned long)p, len);

	return ____memset(p, c, len);
}
...
{
	kasan_unpoison_shadow(object, cache->objsize);
}

void kasan_poison_object_data(struct kmem_cache *cache, void *object)
{
	kasan_poison_shadow(object,
			round_up(cache->objsize, KASAN_SHADOW_SCALE_SIZE),
			KASAN_KMALLOC_REDZONE);
}

void kasan_slab_alloc(struct kmem_cache *cache, void *object)
{
	kasan_kmalloc(cache, object, cache->objsize);
}
...
void kasan_kmalloc(struct kmem_cache *cache, const void *object, size_t size)
{
	...
	redzone_end = round_up((unsigned long)object + cache->objsize,
				KASAN_SHADOW_SCALE_SIZE);
...
void kasan_krealloc(const void *object, size_t size)
{
	...
	if (unlikely(!PageSlab(page)))
		...
		kasan_kmalloc(page->slab, object, size);
}
