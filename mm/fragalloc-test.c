...
static void alloc_work_fn(struct work_struct *work)
{
	run_alloc();
}

static DECLARE_DELAYED_WORK(alloc_work, alloc_work_fn);

static int fragalloc_alloc(void *data, u64 val)
{
	/*
	 * Bypass actual work to workqueue in order to avoid
	 * dead-lock due to network connection
	 */
	schedule_delayed_work(&alloc_work, HZ);
