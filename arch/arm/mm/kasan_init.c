...
extern phys_addr_t lowmem_limit __initdata;
...
void __init kasan_init(void)
{
	...
	for_each_memblock(memory, reg) {
		...
		if (reg->base + reg->size > lowmem_limit)
			end = __va(lowmem_limit);
		if (start >= end)
			break;
	...
	create_mapping((unsigned long)kasan_mem_to_shadow(MODULES_VADDR),
			(unsigned long)kasan_mem_to_shadow(MODULES_END),
			NUMA_NO_NODE);
